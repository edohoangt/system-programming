...
fn read_file_lines(filename: &String) -> Result<Vec<String>, io::Error> {
    ...
    let mut strs_vec: Vec<String> = Vec::new();

    for line in io::BufReader::new(file).lines() {
        let line_str = line?;
        strs_vec.push(line_str);
    }
    Ok(strs_vec)
}
...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
    ...
    let mut dp = Grid::new(seq1_len + 1, seq2_len + 1);
...
fn main() {
    ...
    let file1_lines =
        read_file_lines(filename1).expect(&format!("Error when reading file {}.", filename1));
    let file2_lines =
        read_file_lines(filename2).expect(&format!("Error when reading file {}.", filename2));

    let dp_grid = lcs(&file1_lines, &file2_lines);
