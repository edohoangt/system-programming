...
impl Process {
    ...
    // This function will print details about this process.
    pub fn print(&self) {
        println!(
            "========== \"{}\" (pid {}, ppid {}) ==========",
    ...
    pub fn list_fds(&self) -> Option<Vec<usize>> {
        ...
        let mut fds_vec: Vec<usize> = Vec::new();

        for entry in dir_iter {
            let entry = entry.ok()?;
            let entry_path = entry.file_name().into_string().ok()?;
            fds_vec.push(entry_path.parse::<usize>().ok()?);
        }
